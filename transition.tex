...
$\R \time \R^+$? Still not sure that max and min in defn add anything. }
with
\[
	\kappa_n := \begin{cases}
...
Let $\alpha > \frac{1}{2}$, $C > 0$ and $k_n$ be any sequence satisfying $k_n = \smallO{n^{\frac{1}{2\alpha - 1}}}$. \TM{ Typo? $2\alpha-1$ should be $2\alpha+1$? } 
Then, for all $p \in \Kcal_{C}(k_n)$,
\[
	\mu_{\alpha,\nu ,n}(B_{\Pcal,n}(p)) = \mu_{\alpha,\nu}(B_\Pcal(p))\left(1 - \bigT{k_n^{2\alpha - 1} n^{-(2\alpha - 1)}}\right).
\]
\TM{ I don't follow. Spell it out. }
...
\]
\TM{ Note you cannot really force/assume $p_0 = (0,y)$ is in your PPP $\Pcal_n$. (The summation over $\Pcal_n \setminus (0,y)$ suggests this is what you do.) For taking the expectation of sort of thing, Palm theory / Mecke are used typically. At this point though you can again still just say we add $p_0$ to the PPP. }
...
To further analyze this, let us introduce some notation. For any $p = (x,y) \in \Rcal_n$ we will define the left and right boundary functions as, respectfully,\TM{ respectively? }
\begin{align}
	b_p^-(z) &= \begin{cases}
...
For the remainder of the analysis we restrict ourselves to the case where $x^\prime > 0$. Due to symmetry the situation for $x^\prime < 0$ is the same. \TM{ does this not depend on $p$? } 
