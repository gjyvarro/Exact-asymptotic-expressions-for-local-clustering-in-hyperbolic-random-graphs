...
We will first relate $\gamma(k_n)$ to an integral expression involving $T_\infty(y)$ and $\Exp{c^\ast(k_n;\Gbox)}$ to one involving $\Exp{T_{\text{box}}(y)}$. Recall the definition of $y_{k,C}^\pm$ from~\eqref{eq:def_y_k_C} and the interval $\Kcal_{C}(k_n) = [y_{k_n,C}^-, y_{k_n,C}^+]$. Note that for any $y \in \Kcal_C(k_n)$ it holds that as
...
%Note that $\hat{y}_{\text{left}}(p,p^\prime)$ and $\hat{x}_{\text{left}}(p,p^\prime)$ correspond to, respectively, $\hat{y}(p,p^\prime)$ and $\hat{x}(p,p^\prime)$ considered in Section~\ref{ssec:missing_triangles}. The derivation of $\hat{y}_{\text{right}}(p,p^\prime)$ and $\hat{x}_{\text{right}}(p,p^\prime)$ is done in a similar manner and we omit the details here.
