...
To analyze $T_{\text{box}}(y_0) - T_\infty(y_0)$ we first reiterate that the difference between the indicator $\ind{p_1 \in \BallPon{p}}$ in the finite box model and $\ind{p_1 \in \BallPo{p}}$ is that in $\Gbox$ we identified the boundaries of the interval $[-\frac{\pi}{2}e^{R_n/2}, \frac{\pi}{2} e^{R_n/2}]$ and we stop at height $y = R_n$. It is clear that for any $p = (x,y)$ with $0 \le y \le R_n$ we have that $\BallPon{p} = \BallPo{p} \cap \Rcal$. However, when we take another point $p^\prime \in \BallPon{p}$ then it could happen that there are points in the intersection $\BallPon{p} \cap \BallPon{p^\prime}$ that are not in $\BallPo{p} \cap \BallPo{p^\prime}$. Let us denote this region by $\mathcal{T}_{\Pcal \Delta \Pcal_n}(p,p^\prime)$, see Figure~\ref{fig:comparing_triangles_diff_intersections} for an example of this region. Then, any $p_2 \in \mathcal{T}_{\Pcal \Delta \Pcal_n}(p,p^\prime)$ creates a triangle with $p$ and $p^\prime$ in $\Gbox$ that is not present in $\Ginf$.  
...
Let us write $\Rcal^\prime := \R \times \R_+ \setminus \Rcal$ and let $p_0 = (0,y)$ denote the typical point. Then
\begin{align*}
	\left|T_{\text{box}}(p_0) - T_{\infty}(p_0)\right| 
	&= \sum_{p_1, p_2 \in \Pcal }^{\ne} \ind{p_1 \in \BallPon{p_0}} 
		\ind{p_2 \in \mathcal{T}_{\Pcal \Delta \Pcal_n}(p_0, p_1)} 
...
\begin{align*}
	...
	&\le \int_{\Rcal} \mu\left(\mathcal{T}_{\Pcal \Delta \Pcal_n}(p_0,p_1)\right) f(x_1,y_1) 
		\dd x_1 \dd y_1\\
...
\begin{align*}
	&\hspace{-30pt}\int_{\Kcal_{C}(k_n)} \rho(y,k_n) \left|\Exp{T_{\text{box}}(p_0) - T_{\infty}(p_0)}\right| 
		e^{-\alpha y_0} \dd y_0\\
	&= \bigO{1} n^{-(4\alpha - 2)} k_n^2 \int_{0}^\infty \rho(y_0,k_n) e^{-\alpha y_0} \dd y_0\\
	&= \bigO{1} n^{-(4\alpha - 2)} k_n^2 p_{k_n} = \smallO{s(k_n) p_{k_n} k_n^2},
\end{align*}
%\TM{ Typo : $e^{-\alpha}$ }
where the last part follows from Lemma~\ref{lem:scaling_s_alpha}. This establishes~\eqref{eq:triangle_diff_main_term} and hence finishes the proof.
%
