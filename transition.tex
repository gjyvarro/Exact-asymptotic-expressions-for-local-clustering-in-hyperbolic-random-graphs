...
\] 
Recall that $\Gbox$ is obtained by restricting the Poisson Point Process $\PPP$ to the box $\Rcal = (-I_n, I_n] \times (0, R_n]$, with $I_n = \frac{\pi}{2} e^{R_n/2}$ (we write $\Pcal_n$ for this process), and connecting two points $p_1, p_2 \in \Rcal$ if $|x_1 - x_2|_{\pi e^{R_n/2}} \le e^{(y_1 + y_2)/2}$. We recall that by definition of the norm $|.|_{\pi e^{R_n/2}}$ the left and right boundaries of $\Rcal$ are identified. See Section~\ref{ssec:finite_model} for more details. 
...
We now turn to the task of calculating the expected number of triangles of a node at height $y$, for both the infinite model and the finite box model. Recall that we added a typical point $p_0 = (0,y)$, with exponentially distributed height to the Poisson Point Process. Then if we define
\[
	T_{\Pcal,n}(p) = \sum^{\ne}_{p_1, p_2 \in \Pcal_n \setminus p} T_{\Pcal,n}(p,p_1,p_2)
\]
where
\[
...
it follows that
\[
	\Exp{T_{\Pcal,n}(p_0)} = \frac{1}{2}\iint_{\Rcal^2} T_{\Pcal,n}(p_0,p_1,p_2)
	f(x_1,y_1) f(x_1,y_1) \, dx_1 \, dx_2 \, dy_1 \, dy_2,
\]

\TM{ $p$ is supposed to be $p_0$ I assume. }\PvdH{Indeed. It has been corrected.}
The difference between the indicator $\ind{p_1 \in \BallPon{p}}$ in the finite box model and $\ind{p_1 \in \BallPo{p}}$ is that in $\Gbox$ we identified the boundaries of the interval $[-\frac{\pi}{2}e^{R_n/2}, \frac{\pi}{2} e^{R_n/2}]$ and we stop at height $y = R_n$. It is clear that for any $0 \le y \le R_n$ we have that $\BallPon{p_0} = \BallPo{p_0} \cap \Rcal$. However, when we take another point $p^\prime \in \BallPon{p}$ then it could happen that there are points in the intersection $\BallPon{p} \cap \BallPon{p^\prime}$ that are not in $\BallPo{p} \cap \BallPo{p^\prime}$. Let us denote this region by $\mathcal{T}_{\Pcal \Delta \Pcal_n}(p,p^\prime)$. Then, any $p_2 \in \mathcal{T}_{\Pcal \Delta \Pcal_n}(p,p^\prime)$ creates a triangle with $p$ and $p^\prime$ in $\Gbox$ that is not present in $\Ginf$.  

Define
\begin{equation}\label{eq:def_tilde_triangle_indicator}
	\widetilde{T}_{\Pcal,n}(p_0,p_1,p_2) = \ind{p_1 \in B_{\Pcal,n}(p_0)}\ind{p_2 \in B_{\Pcal,n}(p_0)}\ind{p_2 \in B_{\Pcal}(p_1) \cap \Rcal}
\end{equation}
Then
\[
...
There are two important points in the box $\Rcal$. These are the intersection between the left boundary of $p^\prime$ and the right boundary of $p^\prime$, as it continues from the left side of the box, and the left boundary of $p$. We denote by $(x^\ast(p^\prime), y^\ast(p^\prime))$ the intersection between the left and right boundary of $p^\prime$ and by $(\hat{x}(p,p^\prime), \hat{y}(p,p^\prime))$ the intersection between the left boundary of $p$ and the right boundary of $p^\prime$, see Figure \ref{fig:comparing_triangles_diff_intersections}. 
...
\] 
This solution is $\frac{x^\prime - \pi e^{R_n/2}}{1 + e^{(y^\prime - y)/2}}$ and again $\hat{y}(p,p^\prime)$ is obtained by plugging the solution into either left or right hand side of the equation, yielding $\hat{y}(p,p^\prime) = 2 \log\left(\frac{\pi e^{R_n/2} - x^\prime}{e^{y/2} + e^{y^\prime/2}}\right)$.
...
Let $p_0 = (0,y)$ with $y \in \Kcal_{C}(k_n)$. Then for some $C > 0$, as $n \to \infty$,
\[
...
The proof of the lemma is not difficult but cumbersome, since it involves computing many different integrals. We postpone this proof till the end of this section and proceed with the main goal, proving Proposition~\ref{prop:convergence_average_clustering_P_n}. But first we state a small lemma about the scaling of $s(k_n)$ that will be very useful.  
...
Let $\mathrm{deg}_{\text{box}}(y)$ denote the number of nodes in the ball $\BallPon{y}$ (degree of typical node at height $y$) and similarly $\mathrm{deg}_n(y)$ is the number of nodes in $\BallHyp{y}$. Recall that
\[
	\Exp{c^\ast(k_n; \Gbox)} = \frac{\int_{\Rcal} \Exp{\ind{\mathrm{deg}_{\text{box}}(y) = k_n} T_{\Pcal,n}(y)} f(x,y) \dd x \dd y}{\binom{k_n}{2}\Exp{N_{\text{box}}(k_n)}},
\]
and the definition of $\Kcal_{C,n}(k_n)$
\[
	\Kcal_{C,n}(k_n) = \left\{ (x,y) \in \Rcal \, : \, \frac{k_n - C k_n}{\xi} \le e^{\frac{y}{2}}
		\le \frac{k_n + C \kappa_n}{\xi} \right\},
\]
where for $k_n \to \infty$ we defined $\kappa_n := \sqrt{k_n \log(k_n)}$.

Since 
\[
	\Exp{N_{\text{box}}(k_n)} = \int_{\Rcal} \rho_{\text{box}}(y,k_n) f(x,y) \dd x \dd y,
\]
the Corollary~\ref{cor:concentration_argument_other_models}, using the second statement of Lemma~\ref{lem:concentration_argument_rho_approximation}, implies
\[
	\Exp{N_{\text{box}}(k_n)} = (1+o(1)) \alpha n \int_{0}^\infty \rho(y,k_n) e^{-\alpha} \dd y 
	= (1+\smallO{1}) n \Exp{N_{\infty}(k_n)}.
\]
Moreover, since $\Exp{\ind{\mathrm{deg}_n(y)(y) = k_n} T_{\Pcal,n}(y)} \le \rho_{\text{box}}(y,k_n) k_n^2$ a concentration of heights argument (cf. first statement of Lemma~\ref{lem:concentration_argument_rho_approximation}) yields
\begin{align*}
	&\hspace{-30pt}\int_{\Rcal} \Exp{\ind{\mathrm{deg}_n(y) = k_n} T_{\Pcal,n}(y)} 
		f(x,y) \dd x \dd y\\
	&= (1+\smallO{1})\int_{\Kcal_{C,n}(k_n)} \rho_{\text{box}}(y,k_n) \Exp{T_{\Pcal,n}(y)} f(x,y) \dd x \dd y\\
	&= (1 + \smallO{1}) \alpha n \int_{a_n^-}^{a_n^+} \rho_{\text{box}}(y,k_n) \Exp{T_{\Pcal,n}(y)} e^{-\alpha} \dd x \dd y,
\end{align*}
with $a_n^\pm = 2\log\left(\frac{(k_n \pm C \kappa_n) \wedge \xi_{\alpha, \nu}}{\xi}\right)$. %\TM{ again, this def has to be adapted. } 
Next we note that for $a_n^- \le y \le a_n^+$,
\begin{equation}\label{eq:relation_E_T_Pcal_vs_Delta_P}
	\Exp{T_\Pcal(y)} = \frac{\Mu{\BallPo{y}}^2}{2} P(y) = (1+\smallO{1}) \binom{k_n}{2} P(y)
\end{equation}
and hence it now suffices to show that
\[
	\binom{k_n}{2}^{-1}\int_{a_n^-}^{a_n^+} \rho_{\text{box}}(y,k_n) \Exp{T_{\Pcal,n}(y)} e^{-\alpha y} \dd y
	= (1+\smallO{1}) \int_{a_n^-}^{a_n^+} \rho(y,k_n) P(y) e^{-\alpha y} \dd y. 
\]


We do this in two stages. First note that by a concentration of heights argument
\begin{equation}\label{eq:transition_clustering_main_term}
	\int_{a_n^-}^{a_n^+} \rho_{n}(y,k_n) P(y) e^{-\alpha y} \dd y
	= (1 + \smallO{1}) \int_{a_n^-}^{a_n^+} \rho(y,k_n) P(y) e^{-\alpha y} \dd y.
\end{equation}
It therefore remains toshow that
\begin{equation}\label{eq:transition_clustering_error_term}
	\binom{k_n}{2}^{-1} \int_{a_n^-}^{a_n^+} \rho_{\text{box}}(y,k_n) \left|\Exp{T_{\Pcal,n}(y) - T_{\Pcal}(y)}\right| 
	e^{-\alpha y} \dd y
	= \smallO{1} \int_{a_n^-}^{a_n^+} \rho(y,k_n) P(y) e^{-\alpha y} \dd y.
\end{equation}

For this we write
\begin{align*}
	\left|T_{\Pcal,n}(p) - T_{\Pcal}(p)\right| = \sum_{p_1, p_2 \in \Rcal} \ind{p_1 \in \BallPon{p}} \ind{p_2 \in \mathcal{T}_{\Pcal \Delta \Pcal_n}(p, p_1)} + \sum_{p_1, p_2 \in \Rcal \setminus \Rcal} T_{\Pcal}(p,p_1,p_2)
\end{align*}
so that by the Campbell-Mecke formula
\begin{align*}
	\left|\Exp{T_{\Pcal,n}(p) - T_{\Pcal}(p)}\right|
	&\le \int_{\Rcal} \mu\left(\mathcal{T}_{\Pcal \Delta \Pcal_n}(p,p_1)\right) f(x_1,y_1) 
		\dd x_1 \dd y_1\\
	&\hspace{10pt}+ \BL{\int_{\Rcal \setminus \Rcal}\int_{\Rcal \setminus \Rcal}} T_{\Pcal}(p,p_1,p_2) f(x_1,y_1) f(x_2,y_2)
...
\begin{align*}
	&\hspace{-30pt}\iint_{\Rcal \setminus \Rcal} T_{\Pcal}(p,p_1,p_2) f(x_1,y_1) f(x_2,y_2)
		\dd x_2 \dd y_2 \dd x_1 \dd y_1\\
	&\le \left(\int_{\Rcal \setminus \Rcal} \ind{p_1 \in \BallPo{p}} f(x_1,y_1) \dd x_1 \dd y_1\right)^2
...
\begin{equation}\label{eq:triangle_count_diff_scaling}
	\left|\Exp{T_{\Pcal,n}(p) - T_{\Pcal}(p)}\right| = \bigO{y n^{-(2\alpha - 1)} + n^{-(4\alpha-2)} e^{y}}
	= \bigO{n^{-(4\alpha-2)} e^{y}}.
\end{equation}
Therefore, since $e^{y} = \bigO{1} k_n^2$ on $\Kcal_C(k_n)$, 
\begin{align*}
	&\hspace{-30pt}\binom{k_n}{2}^{-1}\int_{a_n^-}^{a_n^+} \rho(y,k_n) \left|\Exp{T_{\Pcal,n}(p) - T_{\Pcal}(p)}\right| 
		e^{-\alpha y} \dd y\\
	&= \bigO{1} n^{-(4\alpha - 2)} \int_{0}^\infty \rho(y,k_n) e^{-\alpha y} \dd y\\
	&= \bigO{1} n^{-(4\alpha - 2)} k_n^{-(2\alpha + 1)} = \smallO{s(k_n) k_n^{-(2\alpha + 1)} },
\end{align*}
%\TM{ Typo : $e^{-\alpha}$ }
where the last part follows from Lemma~\ref{lem:scaling_s_alpha}. \TM{ Also justify penultimate step! }\PvdH{What do you mean?}
...
From the proof of Proposition~\ref{prop:convergence_average_clustering_P_n} we can extract the following corollary, which will be useful later on in Section~\ref{sec:concentration_c_P_n}.

\begin{corollary}\label{cor:adjusted_triangle_counting_P_n}
Uniformly for all $y \in \Kcal_C(k_n)$, as $n \to \infty$
\[
	\Exp{\widetilde{T}_{\Pcal,n}(y)} = (1+\smallO{1})\Exp{T_\Pcal(y)},
\]
where
\[
	\widetilde{T}_{\Pcal,n}(y) = \sum_{(p_1, p_2) \in \Pcal}^{\ne} 
		\ind{p_1 \in B_{\Pcal,n}(y)}\ind{p_2 \in B_{\Pcal,n}(y)}\ind{p_2 \in B_{\Pcal}(p_1) \cap \Rcal}.
\]

In particular,
\[
	\binom{k_n}{2}^{-1}\int_{\Kcal_{C}(k_n)} \rho_{\text{box}}(y,k_n) \Exp{\widetilde{T}_{\Pcal,n}(y)} f(x,y) \dd x \dd y
	= (1+\smallO{1}) \int_{\Rcal} \rho(y,k_n) P(y) f(x,y) \dd x \dd y. 
\]
\end{corollary}

\begin{proof}
First we note that equation~\eqref{eq:triangle_count_diff_scaling} implies that for $y \in \Kcal_C(k_n)$
\[
	\Exp{\left|\widetilde{T}_{\Pcal,n}(y) - T_\Pcal(y)\right|} = \bigO{n^{-(4\alpha - 2)}k_n^2}.
\]
Next, equation~\ref{eq:relation_E_T_Pcal_vs_Delta_P} together with Proposition~\ref{prop:asymp} yield
\[
	\Exp{T_\Pcal(y)} = (1+\smallO{1})\binom{k_n}{2} P(y) = \bigT{k_n^2 s(k_n)},
\]
on $\Kcal_{C}(k_n)$, with $s(k)$ defined by~\eqref{eq:def_scaling_function}. By Lemma~\ref{lem:scaling_s_alpha} we now have that for all $y \in \Kcal_{C}(k_n)$
\[
	n^{-(4\alpha - 2)}k_n^2 = \smallO{k_n^2 s(k_n)} = \smallO{\Exp{T_\Pcal(y)}}
\]
and hence
\[
	\Exp{\widetilde{T}_{\Pcal,n}(y)} = \Exp{T_\Pcal(y)} + \bigO{\Exp{\left|\widetilde{T}_{\Pcal,n}(y) - T_\Pcal(y)\right|}}
	= (1+\smallO{1})\Exp{T_\Pcal(y)}.
\]

Using this we derive the second statement as follows 
\begin{align*}
	&\hspace{-30pt}\binom{k_n}{2}^{-1}\int_{\Kcal_{C}(k_n)} \rho_{\text{box}}(y,k_n) \Exp{\widetilde{T}_{\Pcal,n}(y)} 
		f(x,y) \dd x \dd y\\
	&=(1 + \smallO{1})\binom{k_n}{2}^{-1}\int_{\Kcal_{C}(k_n)} \rho_{\text{box}}(y,k_n) \Exp{T_\Pcal(y)} 
		f(x,y) \dd x \dd y\\
	&=(1 + \smallO{1})\int_{\Kcal_{C}(k_n)} \rho_{\text{box}}(y,k_n) P(y)f(x,y) \dd x \dd y \\
	&=(1 + \smallO{1})\int_{\Rcal} \rho_{\text{box}}(y,k_n) P(y)f(x,y) \dd x \dd y
\end{align*}
where the third line is due to equation~\eqref{eq:relation_E_T_Pcal_vs_Delta_P} and the last line follows by a concentration of heights argument.
\end{proof}
