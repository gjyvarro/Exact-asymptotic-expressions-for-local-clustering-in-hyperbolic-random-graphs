...
Finally, we note that the height $y$ of a randomly sampled point has the exponential density $\alpha e^{-\alpha y}$. \PvdH{Could someone add an intuition or better explanation why this is the case.} 
...
Note that Theorem~\ref{thm:local_clustering_hyperbolic} states that for any sequence $k_n  = \smallO{n^{1/(2\alpha + 1)}}$
\[
...
\]
Then $k_n = \smallO{n^{1/(2\alpha + 1)}}$ and by Theorem~\ref{thm:local_clustering_hyperbolic}
\[
...
and similarly
\[
	a_n \max_{3 \le t \le a_n} \Exp{\left|\frac{N_{\H,n}(t)}{N_\Pcal(t)} - 1\right|} \to 0.
\]

Then, since $\Exp{N_\Pcal(t)} = \bigT{1} t^{-(2\alpha + 1)}$
\begin{align*}
