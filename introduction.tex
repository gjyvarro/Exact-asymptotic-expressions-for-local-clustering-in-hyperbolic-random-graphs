...
To define the local clustering function, let $D_G(v)$ denote the degree of node $v$ and $N_G(k)$ the number of nodes with degree $k$ in the graph $G$. In addition let $\T_G(v,v_1,v_2) = \ind{(v,v_1) \in E} \ind{(v, v_2) \in E} \ind{(v_1, v_2) \in E}$ be the indicator that $v, v_1$ and $v_2$ form a triangle and write
\begin{equation}
