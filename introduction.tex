...
\]
as $n \to \infty$, where $\gamma(\cdot)$ is as in Theorem~\ref{thm:mainkfixed}.
\end{theorem}
...
\PvdH{ToDo: add remark on maximum scaling for $k_n$.}
...
The remaining sections are devoted to the proof of Theorem~\ref{thm:mainktoinfty}, which turns out to be a lot more involved. In particular, since we push the possible scaling of $k_n$ to its maximum, a great deal of work is needed to properly control the arising error terms. 
