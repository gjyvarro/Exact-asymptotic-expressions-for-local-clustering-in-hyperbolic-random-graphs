...
Our first result shows that the local clustering coefficient of the hyperbolic random graph converges in expectation to a constant $c_\infty$, which we can explicitly write down in terms of the special functions defined above.
...
Let $\alpha > \frac{1}{2}$, $\nu > 0$, $\xi = \frac{4\alpha\nu}{\pi(2\alpha-1)}$ and $(k_n)_{n \ge 1}$ be any positive sequence, possibly constant, such that $k_n \ge 3$ and $k_n = \smallO{n^{\frac{1}{2\alpha + 1}}}$. Then, 
\[
...
\end{corollary}
\MS{Are you sure all constants are correct? I thought for $\alpha = 3/4$ it might be $6\nu/\pi$. But if you are sure, you can remove this comment of course.}
