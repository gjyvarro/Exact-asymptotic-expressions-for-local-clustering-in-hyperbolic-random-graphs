...
Gugelmann et al.~\cite{gugelmann2012random} also showed that the average degree converges in probability to the constant $8\nu\alpha^2/ \pi (2\alpha-1)^2$, and they showed that the (local) clustering coefficient is non-vanishing in the sense that it is bounded below by a positive constant a.a.s. Here, and in the rest of the paper, for a sequence $(E_n)_n$ of events, $E_n$ {\em asymptotically almost surely} (a.a.s.) means that $\Prob{E_n} \to 1$ as $n \to \infty$. Here the events $E_n$ might be defined in different probability spaces, in which case $\Pee$ will the corresponding probability measure.
...
Here we summarize our main results regarding the degrees and clustering in the KPKVB model.

\subsubsection{Degrees}

The first rigorous result on the degree distribution in the KPKVB model was given in~\cite{gugelmann2012random}, where they analyzed the fraction of nodes with degree $k$. Although their results allowed $k = k_n$ to scale with $n$ as $k_n = n^\delta$, it did not cover the full range $1 \le k_n \le n-1$. Our main result for the degrees gives this full characterization of the degree distribution.

\begin{theorem}\label{thm:degrees_hyperbolic}
Let $\alpha >\frac{1}{2}$, denote by $N_n(k)$ the number of vertices with degree $k$ in the KPKVB model $G(n;\alpha,\nu)$ and consider a sequence of integers $k_n$ with $0 \leq k_n \leq n-1$.
\begin{enumerate}
\item If $k_n =\smallO{n^{\frac{1}{2\alpha+1}}}$ as $n\rightarrow \infty$, then a.a.s.
\[
	N_n(k_n) =(1+o(1)) n p_{k_n},
\]
where
\[
	p_{k_n} = \int_0^\infty \Pee(Po(\xi e^{\frac{y}{2}})=k_n)\alpha e^{-\alpha y}dy,
\]
and $p_{k_n}=\frac{2\alpha \xi^{2\alpha}\Gamma^+(k_n-2\alpha,\xi)}{k_n!}=(1+o(1)) 2\alpha \xi^{2\alpha} k_n^{-(2\alpha+1)}$ as $k_n \rightarrow \infty$.
...
The first result of Theorem~\ref{thm:degrees_hyperbolic} states that the fraction of nodes of degree $k_n$ converges to $p_{k_n}$. Not surprisingly, the limit distribution is the same as found in~\cite{gugelmann2012random}. However, their this result was proved only for $k_n \le n^{\delta^\prime}$, where $\delta^\prime < \min\left\{\frac{2\alpha-1}{4\alpha(2\alpha +1)}, \frac{2(2\alpha -1)}{5(2\alpha + 1)}\right\} < \frac{1}{2\alpha + 1}$. Therefore, our result extends theirs by showing that this converges holds all the way till $n^{\frac{1}{2\alpha + 1}}$ . Moreover, we establish what happens at the boundary, where $k_n$ scales as $n^{\frac{1}{2\alpha + 1}}$ and show that this scaling is indeed the bound for the distribution since $N_n(k_n) = 0$ a.a.s. when $k_n$ grows faster than $n^{\frac{1}{2\alpha + 1}}$.

\subsubsection{The clustering coefficient}

Our first result on the clustering shows the convergence of the local clustering coefficient in the KPKVB model and establishes the limit exactly.
...
\]
\[
	\lim_{n \to \infty} \Exp{\left|\frac{c(k;G_n)}{\gamma(k)} - 1\right|} = 0.
\]
...
There are a few other observation to be made regarding our results on clustering.
...
\]
Then $b_n \le a_n \ll n^{\frac{1}{2\alpha + 1}}$ and therefore by Theorem~\ref{thm:mainktoinfty}
\[
...
Our results for the clustering function in the KPKVB model are valid for any sequence of non-negative integers $k_n \to \infty$ such that $k_n \ll n^{\frac{1}{2\alpha + 1}}$. Although one would like to have results for any sequence $k_n \le n-1$, it turns out that $n^{1/(2\alpha + 1)}$ is the near maximum scaling for which Theorem~\ref{thm:clustering_coefficient_hyperbolic} can be true. To see why this is the case note that by definition of the local clustering function~\eqref{eq:def_clustering_function} we have that $c(k_n; G_n) = 0$ if $N_{n}(k_n) = 0$, where $N_n(k_n)$ denotes the number of vertices in a KPKVB graph $G(n; \alpha, \nu)$ with degree $k_n$. It follows by Markov's inequality that for any positive function $f : \R_+ \to \R_+$
\begin{align*}
...
It follows from Proposition~\ref{prop:asymp} that there is a transition in the scaling of the local clustering function at $\alpha = 3/4$. This corresponds to an exponent $5/2$ for the probability mass function of the degree distribution. This transition is different from those often observed for networks with scale-free degree distributions, where transitions occur at integer values of the exponent. At this point, it is unclear what the underlying reason is for the appearance of the transition at this half exponent. Interestingly, a similar transition point has also been observed for both majority vote models~\cite{chen2015critical} and flocking dynamics~\cite{miguel2018effects} on networks with scale-free degree degree distributions.
