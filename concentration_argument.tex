...
%
%
% with $\hat{\mu}_n(y) = (1 + \phi_n(y))\mu(y)$ and 
...
%We note that the statement will follow from Lemma~\ref{lem:concentration_argument} if we can show that
%\[
%	\int_0^R \hat{\rho}_n(y,k_n) e^{-\alpha y} \dd y
%	= \left(1 + o(1)\right) \int_0^R \rho(z,k_n) e^{-\alpha z} \dd z.
%\] 
%or equivalently,
%\[
%	\int_0^R \rho(z,k_n) e^{-\alpha z} \dd z
%	= \left(1 + o(1)\right)\int_0^R \hat{\rho}_n(y,k_n) e^{-\alpha y} \dd y.
%\]
%
%We will do this in two steps.
%
%Therefore, what is left is to show that
%\[
%	\int_0^{(1-\varepsilon^\prime)R} \hat{\rho}_n(y,k_n) e^{-\alpha y} \dd y
%	= \left(1 + o(1)\right) \int_0^{(1-\varepsilon^\prime)R} \rho(z,k_n) e^{-\alpha z} \dd z.
%\] 
%or equivalently,
%\[
%	\int_0^{(1-\varepsilon^\prime)R} \rho(z,k_n) e^{-\alpha z} \dd z
%	= \left(1 + o(1)\right)\int_0^{(1-\varepsilon^\prime)R} \hat{\rho}_n(y,k_n) e^{-\alpha y} \dd y.
%\]
%
%Using the change of variable $z = \mu^{-1}(\hat{\mu}_n(y))$ and writing $\hat{a}_n^\pm = \hat{\mu}_n^{-1}(\mu(a_n^\pm))$ and $\hat{R}_{\varepsilon^\prime} := \hat{\mu}_n^{-1}(\mu((1-\varepsilon^\prime)R))$, we get
%\begin{align*}
%	\int_{a_n^+}^{(1-\varepsilon^\prime)R} \rho(z,k_n) e^{-\alpha z} \dd z 
%	&= \int_{a_n^+}^{(1-\varepsilon^\prime)R} \Prob{\Po(\mu(z)) = k_n} e^{-\alpha z} \dd z\\
%	&= \int_{\hat{a}_n^+}^{\hat{R}_{\varepsilon^\prime}} \Prob{\Po(\hat{\mu}_n(y)) = k_n} e^{-\alpha \mu^{-1}(\hat{\mu}_n(y))} 
...
\]
%Let $\eps \in (0, 1)$. Then for all $0 \le y \le (1 - \eps)R$
%\[
...
%where
%\[
%	\phi_{\H,n}^{(1)}(y) = \frac{2\alpha - 1 - 4\pi}{4\pi} e^{-(\alpha - \frac{1}{2})(R - y)} 
%		+ \left(\alpha - \frac{1}{2}\right)\pi e^{-(\alpha - \frac{1}{2})R - y/2},
%\]
%and
%\[
%	\phi_{\H,n}^{(2)}(y) = 
%	+ \begin{cases}
%			\frac{(2\alpha - 1)K}{3 - 2\alpha}\left(e^{-(\alpha - \frac{1}{2})(R - y)} - e^{-(R - y)}\right)
%			&\mbox{if } 1/2 < \alpha < 3/2,\\
%			\frac{(2\alpha -1)K}{2} (R - y)e^{-(R - y)} &\mbox{if } \alpha = 3/2,\\
%			\frac{(2\alpha - 1)K}{2\alpha - 3} \left(e^{-(R - y)} - e^{-(\alpha - \frac{1}{2})(R - y)}\right)
%			&\mbox{if } \alpha > 3/2,
%		\end{cases}
%\]
