...
\TM{ Maybe state where $\Delta$ is defined in the paper. }
...
\subsection{Concentration \TM{ "of heights"? } argument for the infinite model}
...
\end{equation}
\TM{ taking max with 0 and min with $R$ do not seem necessary. }
...
I would have expected we just integrate wrt.~$y$ and replace $f_{\alppha,\nu}(x,y)$ by $\alpha e^{-\alpha y}$, the exponential density.}
it is enough to consider integration \BL{over} $\Kcal_C(k_n)$ instead of $\Rcal_n$. More precisely, for any $\ell_n = (1 + \smallO{1})k_n$ it is enough to consider $\Kcal_C(\ell_n)$. The flexibility of using $(1+\smallO{1})k_n$ instead of $k_n$ will prove useful in later sections.
...
$a^-$ is undefined! Need to change the definitions. }
...
We shall now bound the terms $\rho(a_n^\pm,k_n)$, starting with $\rho(a_n^+,k_n)$.  Using that $k! > \sqrt{2\pi} k^{k + 1/2} e^{-k}$ \TM{ Reference! } we write
\begin{align*}
...
\TM{ again, in general we cannot neatly split into the two cases $k\to\infty$ and $k=\Theta(1)$ } in which case $\kappa_n = \sqrt{\ell_n \log(\ell_n)}$. Since 
\[
...
\TM{  the term ``average degree" does not fit very well with what goes on in this section. We are considering the expected number of points in a ball, but not yet computing the average degree of the graph. }
...
\]
for $y + y^\prime < R_n$. \TM{ Say what $K$ is and what the restrictions on $r, r'$ are. } This result enables us to determine the measure of a ball around a given point $p=(0,y)$ which will be fairly useful in our subsequent analysis. 
...
\]
\TM{ maybe remind the reader just before of the rather counter intuitive definition of $\BallHyp{0,y}$, which 
lives in the box model not in the original hyperbolic disk. }
where
\[
...
\begin{proof}
We will split the computation of $\Mu{\BallHyp{0,y}}$ into the cases $y^\prime > R_n - y$ and $y^\prime \le R_n - y$ \TM{ You have not said what $y^\prime$ is! } where for the latter we utilize Lemma~\ref{lem:asymptotics_Omega_hyperbolic}. Recall that $\Mu{\BallPo{0,y}} = \xi_{\alpha,\nu} e^{y/2}$ where $\xi_{\alpha,\nu} = \frac{4\alpha\nu}{\pi(2\alpha - 1)}$.
...
\] 
\TM{ There a rogue subscript $n$ in $|x'|_n$ }
...
\subsection{Concentration \TM{ OF HEIGHTS? }argument for hyperbolic and finite box model}
...
\begin{enumerate}
\item for any sequence of functions $h_n(y) : \R_+ \to \R$ such that $h_n(y) = \hat{\rho}_n(y,k_n) \bigO{k_n^s e^{\beta y}}$ \TM{ Why not adjust $h$ so that we incorporate $\hat\rho$ in line below. Then both items look more similar. }  for some $s < 2\alpha(2\alpha + 1)$ and $\beta < \alpha$, we have,
\[
...
\]
\end{enumerate}
\TM{ Say in the lemma statement where def of $K_C$ can be found. }
\end{lemma}
...
\begin{equation}\label{eq:concentration_argument_variable_change}
	\hat{\mu}_n(y) = \mu(z),
\end{equation}
which implies \TM{ "implies"? this is just by definition. }
\[
...
\[
	\mu^{-1}(y) = 2 \log\left(\frac{y}{\xi_{\alpha,\nu}}\right),
\]
\TM{ repeat of previous eq. Typo or leftover of old version ... ? }
...
\end{equation}
\TM{ Please elaborate! Why would $\hat\mu$ be diff'ble anyway? }
Denote by $g^\prime(z)$ the derivative of a function $g$.
\TM{ I would remove this last sentence. }
...
\begin{align*}
	...
	&\le (1 + \smallO{1})k_n^s\int_{a_n^+}^{R_n} \rho(y,k_n)
			e^{(\beta-\alpha) z} \dd z.
\end{align*}
\TM{ Please elaborate 2nd equlaity. I don't quite follow. }
...
\begin{align*}
	...
	&= \left(1 + o(1)\right) \int_{a_n^-}^{a_n^+} \rho(z,k_n) h(z) e^{-\alpha z} \dd z.
\end{align*}
\TM{ again, please elaborate 2nd eq. (in the paper) }
...
Let $\hat{\rho}_n(y,k)$ be any of the two distribution functions $\rho_{\HP,n}(y,k)$ and $\rho_{n}(y,k)$. Then, for any function $g_n(y)$ such that $g_n(y) = \bigO{n^{-1} k_n^{s} e^{\beta y}}\hat{\rho}_n(y,k_n)$ \TM{ Where did the $n^{-1}$ come from? 
Does not appear in L3.4. }, as $n \to \infty$, for some $s < 2\alpha(2\alpha + 1)$ and $\beta < \alpha$,
\[
	\int_{\Rcal_n} g_n(y) f_{\alpha,\nu}(x,y) \dd x \dd y
	= (1+\smallO{1})\int_{\Kcal_C(k_n)} g_n(y) f_{\alpha,\nu}(x,y) \dd x \dd y,
\]
