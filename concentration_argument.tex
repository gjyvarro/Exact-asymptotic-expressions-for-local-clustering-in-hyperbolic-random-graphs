...
$a^-$ is undefined! Need to change the definitions. }\PvdH{You are right. I changed it by taking the minimum with $\kappa_n$.}
...
\begin{enumerate}
\item for any sequence of continuous functions $h_n(y) : \R_+ \to \R$ such that $h_n(y) = \hat{\rho}_n(y,k_n) \bigO{k_n^s e^{\beta y}}$ \TM{ Why not adjust $h$ so that we incorporate $\hat\rho$ in line below. Then both items look more similar. }\PvdH{I am not sure what you are suggesting. Could you explain a bit more?}  for some $s < 2\alpha(2\alpha + 1)$ and $\beta < \alpha$, we have,
\[
...
\begin{equation}\label{eq:concentration_argument_variable_change}
	\hat{\mu}_n(y) = \mu(z),
\end{equation}
and therefore %\TM{ "implies"? this is just by definition. }
\[
...
\end{equation}
\TM{ Please elaborate! Why would $\hat\mu$ be diff'ble anyway? }\PvdH{I have added the diff'ble assumption to the lemma statement}
...
\begin{align*}
	...
	&\le (1 + \smallO{1})k_n^s\int_{a_n^+}^{R_n} \rho(y,k_n)
			e^{(\beta-\alpha) z} \dd z.
\end{align*}
\TM{ Please elaborate 2nd equlaity. I don't quite follow. }\PvdH{The fraction $\hat{\mu}_n^\prime(z)/\mu^{\prime}(z)$ comes from the change of variables. Or is this not what you mean?}
...
Does not appear in L3.4. }\PvdH{I will elaborate on this once the statement of Lemma~\ref{lem:concentration_argument} is updated}, as $n \to \infty$, for some $s < 2\alpha(2\alpha + 1)$ and $\beta < \alpha$,
\[
	\int_{\Rcal_n} g_n(y) f_{\alpha,\nu}(x,y) \dd x \dd y
	= (1+\smallO{1})\int_{\Kcal_C(k_n)} g_n(y) f_{\alpha,\nu}(x,y) \dd x \dd y,
\]
