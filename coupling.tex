...
\]
Then by Lemma~\ref{lem:concentration_argument_rho_approximation} and a concentration of heights argument 
...
\[
	\Kcal_C(k_n) = \left\{y \in \R_+ : \frac{k_n - C \kappa_n}{\xi} \vee 0 \le e^{\frac{y}{2}}
	\le \frac{k_n + C \kappa_n}{\xi} \wedge e^{R_n/2} \right\},
\]
with $C > 0$ and 
\[
...
\] 
As $k_n = O(n^{\frac{1}{2\alpha +1}})$, we have $k_n^{2\alpha +1} =O(n)$ and 
