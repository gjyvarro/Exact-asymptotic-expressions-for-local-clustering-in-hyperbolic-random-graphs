...
We can now use a concentration of heights argument to show that the integration of the Poisson probabilities $\Prob{\Po(\mu_{\Po}(y)) = k_n}$ over $0 \le y \le (1-\varepsilon) R$ is asymptotically equivalent to $p_{k_n}$. And the same holds if we instead consider $\mu_{\mathrm{box}}(y)$.
...
Recall that $N_{Po}(k)$ denotes the number of degree $k$ vertices in $\GPo$. % and $k_n \rightarrow \infty$ as 
...
For the integration over $\Fcal_\epsilon(k_n)$, recall that by Lemma~\ref{lem:probdegFact}, $$\varphi(\{p_1,\dots,p_{r+1}\};p_1,\dots,p_s)=(1+o(1))\varphi(\{p_1,\dots,p_r\};p_1,\dots,p_s)\varphi(\{p_{r+1}\};p_1,\dots,p_s)+\bigO{k_n^{-C}}.$$% =(1+o(1)) \varphi(p_1,\dots,p_r)\varphi(y_{r+1}).$$
...
\begin{proof}[Proof of Theorem~\ref{thm:degrees_hyperbolic}] \hfill\\
