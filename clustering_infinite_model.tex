...
Note that when writing $P(y_0)$ as in integral as in equation \eqref{eq:def_delta_p}, by symmetry in the integration 
...
some algebra, the expression for $c_\Pcal$
\begin{align*}
	\gamma &= J-I^{(0)}-I^{(1)} \\
...
\]
and in particular the function $P(y)$. We therefore start with establishes the asymptotic behavior of the latter. First we combine~\eqref{eq:degree_distribution_P_asymptotics} and~\eqref{eq:general_integral_rho_y_k} to obtain the following scaling result
...
\[
	\Delta_{\Pcal}(y) \sim e^{-\frac{y}{2}(4\alpha - 2)} c_\alpha,
\]
\item for $\alpha = \frac{3}{4}$,
\[
	\Delta_\Pcal(y) \sim \frac{y}{2} e^{-\frac{y}{2}},
\]
\item and for $\alpha > \frac{3}{4}$,
\[
	\Delta_{\Pcal}(y) \sim e^{-\frac{y}{2}} \frac{4\alpha - 3}{4(\alpha - 1)}.
\]
\end{enumerate}
\end{proposition}
...
By Lemme~\ref{lem:Paneq1} we get that
\begin{align*}
...
\[,
	\frac{2^{-4 \alpha-1} (3 \alpha - 1)}{\alpha (\alpha - 1)^2} 
			+ \frac{(\alpha - 1/2 ) B^-(1/2; 1 + 2 \alpha, -2 + 2 \alpha)}{2(\alpha - 1) \alpha}
			- \frac{B(2\alpha, 3-4\alpha)}{4(\alpha - 1)} = c_\alpha,
\]
as defined in Theorem~\eqref{thm:mainktoinfty}. The proof now follows since for $1/2 < \alpha < 3/4$, the remaining three terms go to zero as $y \to \infty$.
...
\]
It can now conclude that all terms in $\frac{2}{y} e^{\frac{y}{2}}\Delta_{\Pcal}(y)$ except the first one are $\smallO{1}$ as $y \to \infty$. By writing $z = e^{-\frac{y}{2}}$ we can rewrite this term as
\[
	\frac{2}{y}\frac{B^-(1-e^{-\frac{y}{2}}, 2\alpha, 3 - 4\alpha)}{4(\alpha - 1)} 
...
\[
	\frac{2}{y}e^{\frac{y}{2}}P(y) \sim \frac{y}{2} e^{-\frac{y}{2}},
\]
as $y \to \infty$.
...
\] 
We will prove that, as $k \to \infty$,
...
\end{equation}
Note that this implies that for any bounded function $\varphi(y)$ satisfying $\lim_{y \to \infty} |\phi(y)| = 0$,
\begin{equation}\label{eq:asymptotics_average_clustering_error_term}
	\int_0^\infty \varphi(y) P(y) \rho(y,k) \alpha e^{-\alpha y} \dd y
	= \smallO{1} \int_0^\infty P(y) \rho(y,k) \alpha e^{-\alpha y} \dd y.
\end{equation}

To establish~\eqref{eq:asymptotics_average_clustering_concentration} recall that $\mu(y) = \xi e^{\frac{y}{2}}$ and consider $\rho(y,k)$ as a function of $y$. Then, since $\mu^\prime(y) = \mu(y)/2$, we get that
\[
	\frac{\partial \rho}{\partial y} = \frac{1}{2}\left(k - \mu(y)\right)\rho(y,k),
\]
...
\end{align*}
as $k \to \infty$. We will show that
\[
...
\begin{align*}
	\rho(a_k^+,k) &= \frac{\mu(a_k^+)^{k}}{k!} e^{-\mu(a_k^+)} \\
	&\sim (2\pi)^{-1/2} k^{-1/2} \left(\frac{\mu(a(k)^+}{k}\right)^{k} e^{-(\mu(a(k)^+ - k)}\\
	&= (2\pi)^{-1/2} k^{-1/2} 
		e^{-k\left(\frac{\mu(a_k^+)}{k} - 1 - \log\left(\frac{\mu(a_k^+)}{k}\right)\right)}.
\end{align*}
Since 
\[
	\frac{\mu(a_k^+)}{k} = 1 + C \sqrt{\frac{\log(k)}{k}}
\]
...
\[
	\int_0^\infty P(y) \rho(y,k) \alpha e^{-\alpha y} \dd y
	= (1+\smallO{1}) \int_{a^-(k)}^{a^+(k)} \Delta_{\Pcal}(y) \rho(y,k) \alpha e^{-\alpha y} \dd y,
\]
as $k \to \infty$. 


We now proceed with the proof of Proposition~\ref{prop:asymptotics_P}, which is split over the different cases for $\alpha$.
...
\[
	\frac{\int_0^{\infty} e^{-(4\alpha - 2)y/2} \rho(y,k) \alpha  e^{-\alpha y} \dd y}
	{\int_0^\infty \rho_{y}(k) \alpha e^{-\alpha y} \dd y} \\
	\sim \xi_{\alpha,\nu}^{4\alpha - 2} k^{-4\alpha + 2}.
\]
This follows immediately from~\eqref{eq:general_clustering_integral_scaling} with $\beta = 2\alpha - 1$.

\paragraph{Proof when \bm{$\alpha = 3/4$}}
Similar to the previous case Proposition~\ref{prop:asymptotics_P} and~\eqref{eq:asymptotics_average_clustering_error_term} imply that it is enough to show that
\[
	\frac{\int_0^{\infty} P(y) \rho(y,k) \alpha  e^{-\alpha y} \dd y}
	{\int_0^\infty \rho_{y}(k) \alpha e^{-\alpha y} \dd y}
	\sim \xi_{\alpha,\nu} \log(k) k^{-1},
\]
This does not follow immediately from~\eqref{eq:general_clustering_integral_scaling} because of the additional logarithmic term. To establish the above result we first show that
\begin{equation}\label{eq:average_clustering_proof_34_main}
	\int_{a(k)^-}^{a(k)^+} P(y) \rho(y,k) \alpha e^{-\alpha y} \dd y
	\sim \log(k) \int_{a(k)^-}^{a(k)^+} e^{-y/2} \rho(y,k) \alpha e^{-\alpha y} \dd y.
\end{equation}
...
\]
it follows from~\eqref{eq:average_clustering_proof_34_main} that
\begin{align*}
	\frac{\int_0^\infty P(y) \rho(y,k) \alpha e^{-\alpha y} \dd y}
		{\int_0^\infty \rho(y,k) \alpha e^{-\alpha y} \dd y}
...
Again, by Proposition~\ref{prop:asymptotics_P} and~\eqref{eq:asymptotics_average_clustering_error_term} it is enough to show that
\[
	\frac{\int_0^{\infty} e^{-y/2} \rho(y,k) \alpha  e^{-\alpha y} \dd y}
	{\int_0^\infty \rho_{y}(k) \alpha e^{-\alpha y} \dd y} 
	\sim \xi_{\alpha,\nu} k^{-1},
\]
which follows from~\eqref{eq:general_clustering_integral_scaling} with $\beta = 1/2$.
