...
\begin{proofof}{Lemma~\ref{lem:triangle_prob_y_coordinates}}
...
\begin{align*}
	...
	&= 4 (\alpha-1/2)^2 \cdot \left( \int_{1>z_1>z_0,z_2>0}  z_0 z_1^{2\alpha-3} z_2^{2\alpha-2} 
		 \dd z_2 \dd z_1 \right. \\
	&\hspace{10pt}\left. - \int_{\substack{{1>z_1>z_0,z_2>0}\\{z_1 < z_0+z_2}}} G(z_1,z_0,z_2) z_0 z_1^{2\alpha-3} 	
...
\[
	J_{a,b,c}^\prime(z_0) := z_0^a \int_{\substack{{1>z_1>\max(z_0,z_2); z_0,z_2>0}\\{z_1 < z_0+z_2}}} z_1^{b+2\alpha-2} z_2^{c+2\alpha-2} \dd z_2 \dd z_1
\]
and write
...
\subsubsection{Explicit expressions for \texorpdfstring{$\gamma, \gamma(k)$}{gamma, gamma(k)} when \texorpdfstring{$\alpha=1$}{alpha equals 1}.}\label{ssec:alphais1}




We've already established that $\gamma, \gamma(k)$ can be obtained at $\alpha=1$ by
taking the $\alpha\to 1$ limit of the expression obtained for $\alpha=1$. Here we derive an alternative explicit expression for completeness. Since the rest of our 
proofs do not the depend on it the reader could decide to skip this section on a first reading.
...
\frac{1}{2}e^{-y}\Li_2(e^{-y})
	\end{align*}
	where $\Li_2(z)=\int_0^z \frac{\ln(1-t)}{t}dt$ is the dilogarithm function.
\end{lemma}
