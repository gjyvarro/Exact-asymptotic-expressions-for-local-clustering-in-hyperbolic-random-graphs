...
As $N\isd\Po(n)$, by Chebyschev's inequality, we have that $|N-n| < \eps n$ a.a.s.
...
%Under the standard coupling of $G_n$ and $\GPo$, we have
For every kixed $k\geq 2$, we have
...
Another application of Cambell-Mecke shows that 
...
$\Var X = \Ee X(X-1) + \Ee X - \left(\Ee X\right)^2 = o\left( \left(\Ee X\right)^2 \right)$.
 By Chebyschev's inequality, we therefore have 
...
\noindent
where the convergence in probability can be justified using Slutksy's theorem together 
