...
%&\leq \frac{|V_n(k)\backslash V_{n,\leq H}(k)|+X_2+|N_{n,\leq H}(k)-N_n(k)|}{a_1(k)n}
\end{align*}
...
In a similar fashion, by the Campbell-Mecke formula,
\begin{align*}
...
\end{align*}
Therefore, $\Pee(|c_n-\gamma|\geq \epsilon) \leq \Pee(\left|\sum_{k=2}^{K_\delta} c_n(k)p_n(k) -\sum_{k=2}^{K_\delta} \gamma(k)p_k\right| \geq \epsilon - 2\delta) \rightarrow 0$ because $c_n(k) \xrightarrow{\Pee} \gamma(k)$ for $k=2,\dots,K_\delta$ and $p_n(k) \xrightarrow{\Pee} p_k$ together imply that $\sum_{k=2}^{K_\delta} c_n(k)p_n(k) \xrightarrow{\Pee} \sum_{k=2}^{K_\delta} \gamma(k) p_k$. %p_k =  = \Theta(k^{-(2\alpha +1)})
\end{proofof}
