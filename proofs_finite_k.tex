...
$\Prob{|c(k;G(n;\alpha,\nu)) - c(k;\GPo(n;\alpha,\nu))| > \sqrt{\eps} } = O(\sqrt\eps)$.
Sending $\eps\searrow 0$ proves the lemma. \PvdH{I think we need to be a bit more precise here and set $\varepsilon = \varepsilon_n := Q n^{-1}$.
}
\end{proof}
...
\PvdH{These notations are not in sync with the rest of the paper.}
Then,
$c_{\leq H}(k) = (1+o(1))\int_0^H \int_0^H \int_0^H \Delta_H(y) \tau(y,k)\rho(y_1)\rho(y_2)\rho(y) dy_1dy_2 dy$ a.a.s.
\PvdH{I do not understand how a.a.s. is proven.}
\end{lemma}
\begin{proof}
...
\end{align*}
\PvdH{What is $E_R$ and what is $D_H$?}
...
\PvdH{Where is the proof of Theorem~\ref{thm:mainkfixed}?}
